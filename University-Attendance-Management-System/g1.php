...
$row = mysqli_fetch_assoc(mysqli_query($conn, "SELECT state_g1 FROM state_g1 LIMIT 1"));
// خزن القيمه بتاعت الموقع يفتح ولا لا وحط ان الافتراضى يكون صفر
$state_g1 = $row ? $row['state_g1'] : 0;
...
if ($state_g1 == 1) { // هنا لو الدكتور لالفعل حط واحد يعنى فتح الموقع
...
// لو ضغط ارسال هناخد بياناته علشان اخزنها فى الداتا بز
    if(isset($_POST['sub'])){
       ...
       // تنظيف المدخلات
        ...
        $sqll =mysqli_query($conn, "SELECT course_g1 FROM state_g1 LIMIT 1 ");
        while($row  = mysqli_fetch_assoc($sqll)){
        $course = $row ['course_g1'];
        
    }
       $sl =mysqli_query($conn, "SELECT loc_g1 FROM state_g1 LIMIT 1 ");
      ...
      // اضافة البيانات فى الداتا بيز
        ...
        $stmt = $conn->prepare("INSERT INTO group1 (name_g1, id_g1, course_g1, loc_g1) VALUES (?, ?, ?, ?)");
 ...
 // تعريف البعد المتاح  قبل استخدامه
    ...
    $lo = mysqli_query($conn, "SELECT loc_g1 FROM state_g1 LIMIT 1");
...
<div class='container'>
        ...
        <section class='group-section'>
            <h2 class='group-title'>The absence of The Group 1</h2>
<h1>Group 1</h1>
                <hr>
...
<head>
    ...
    <link rel='shortcut icon' href='https://i.postimg.cc/0y44MsH1/Picsart-25-11-27-23-47-53-304.png' type='image/x-icon' />
            ...
            <body>
            <div class='container'>
        <h1>Sections Group 1</h1>
        <div class='sections-container'>
      <img src='section.jpg' alt='sections' class='sections-img'>
      <div class='buttons-box'>
        <h1 class='page-title'>Sections — Group 1</h1>
        <a href='g1_c1.php'><button class='section-btn'>Section 1</button></a>
        <a href='g1_c2.php'><button class='section-btn'>Section 2</button></a>
        <a href='g1_c3.php'><button class='section-btn'>Section 3</button></a>
        <a href='g1_c4.php'><button class='section-btn'>Section 4</button></a>
      </div>
    </div>
    </body>
</html>
