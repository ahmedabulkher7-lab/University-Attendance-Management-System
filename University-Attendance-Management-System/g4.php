...
$row = mysqli_fetch_assoc(mysqli_query($conn, "SELECT state_g4 FROM state_g4 LIMIT 1"));
// خزن القيمه بتاعت الموقع يفتح ولا لا وحط ان الافتراضى يكون صفر
$state_g4 = $row ? $row['state_g4'] : 0;
...
if ($state_g4 == 1) { // هنا لو الدكتور لالفعل حط واحد يعنى فتح الموقع
...
// لو ضغط ارسال هناخد بياناته علشان اخزنها فى الداتا بز
    if(isset($_POST['sub'])){
       ...
       // تنظيف المدخلات
        ...
        $sqll =mysqli_query($conn, "SELECT course_g4 FROM state_g4 LIMIT 1 ");
        while($row  = mysqli_fetch_assoc($sqll)){
        $course = $row ['course_g4'];
        
    }
       $sl =mysqli_query($conn, "SELECT loc_g4 FROM state_g4 LIMIT 1 ");
      ...
      // اضافة البيانات فى الداتا بيز
        ...
        $stmt = $conn->prepare("INSERT INTO group4 (name_g4, id_g4, course_g4, loc_g4) VALUES (?, ?, ?, ?)");
 ...
 // تعريف البعد المتاح  قبل استخدامه
    ...
    $lo = mysqli_query($conn, "SELECT loc_g4 FROM state_g4 LIMIT 1");
    while($row = mysqli_fetch_assoc($lo)){
        $locNum = $row['loc_g4'];
// هنا بجيب الفيم من بتاعت اللوكيشن من الداتا بيز 
        if($locNum == 101){
            $loc = mysqli_fetch_assoc(mysqli_query($conn, "SELECT loc1_101, loc2_101 FROM location_b LIMIT 1"));
            $storedLat = $loc['loc1_101'];
            $storedLon = $loc['loc2_101'];
        }
        elseif($locNum == 118){
            $loc = mysqli_fetch_assoc(mysqli_query($conn, "SELECT loc1_118, loc2_118 FROM location_b LIMIT 1"));
            $storedLat = $loc['loc1_118'];
            $storedLon = $loc['loc2_118'];
        }
        elseif($locNum == 318){
            $loc = mysqli_fetch_assoc(mysqli_query($conn, "SELECT loc1_318, loc2_318 FROM location_b LIMIT 1"));
            $storedLat = $loc['loc1_318'];
            $storedLon = $loc['loc2_318'];
        }
        elseif($locNum == 301){
            $loc = mysqli_fetch_assoc(mysqli_query($conn, "SELECT loc1_301, loc2_301 FROM location_b LIMIT 1"));
            $storedLat = $loc['loc1_301'];
            $storedLon = $loc['loc2_301'];
        }
    }
...
<div class='container'>
        ...
        <section class='group-section'>
            <h2 class='group-title'>The absence of The Group 4</h2>
<h1>Group 4</h1>
                <hr>
...
<head>
    ...
    <link rel='shortcut icon' href='https://i.postimg.cc/0y44MsH1/Picsart-25-11-27-23-47-53-304.png' type='image/x-icon' />
            ...
            <body>
            <div class='container'>
        <h1>Sections Group 4</h1>
        <div class='sections-container'>
      <img src='section.jpg' alt='sections' class='sections-img'>
      <div class='buttons-box'>
        <h1 class='page-title'>Sections — Group 4</h1>
        <a href='g4_c1.php'><button class='section-btn'>Section 1</button></a>
        <a href='g4_c2.php'><button class='section-btn'>Section 2</button></a>
        <a href='g4_c3.php'><button class='section-btn'>Section 3</button></a>
        <a href='g4_c4.php'><button class='section-btn'>Section 4</button></a>
      </div>
    </div>
      </body>
</html>
