...
$row = mysqli_fetch_assoc(mysqli_query($conn, "SELECT state_g3_c3 FROM state_g3 LIMIT 1"));
// خزن القيمه بتاعت الموقع يفتح ولا لا وحط ان الافتراضى يكون صفر
$state_g3_c3 = $row ? $row['state_g3_c3'] : 0;
...
if ($state_g3_c3 == 1) { // هنا لو الدكتور لالفعل حط واحد يعنى فتح الموقع
...
// لو ضغط ارسال هناخد بياناته علشان اخزنها فى الداتا بز
    if(isset($_POST['sub'])){
       ...
       // تنظيف المدخلات
        ...
        $sqll =mysqli_query($conn, "SELECT course_g3_c3 FROM state_g3 LIMIT 1 ");
        while($row  = mysqli_fetch_assoc($sqll)){
        $course = $row ['course_g3_c3'];
        
    }
       $sl =mysqli_query($conn, "SELECT loc_g3_c3 FROM state_g3 LIMIT 1 ");
      ...
      // اضافة البيانات فى الداتا بيز
        ...
        $stmt = $conn->prepare("INSERT INTO group3 (name_g3_c3, id_g3_c3, course_g3_c3, loc_g3_c3) VALUES (?, ?, ?, ?)");
 ...
 // تعريف البعد المتاح  قبل استخدامه
    ...
    $lo = mysqli_query($conn, "SELECT loc_g3_c3 FROM state_g3 LIMIT 1");
    while($row = mysqli_fetch_assoc($lo)){
        $locNum = $row['loc_g3_c3'];
...
<head>
    ...
    <link rel='shortcut icon' href='https://i.postimg.cc/0y44MsH1/Picsart-25-11-27-23-47-53-304.png' type='image/x-icon' />
        ...
        <div class='card'>
            <h2 class='title'>Group 3 Section 3</h2>
            <div class='line'></div>
