...
<div class="page-container">
    ...
    <h1 class="main-title">Table For Group 2</h1>
...
if(isset($_POST['sub'])){
    $sel = $_POST['se'];
    $sql = mysqli_query($conn, "SELECT name_g2, id_g2, loc_g2, course_g2, tim FROM group2 WHERE course_g2='$sel' ORDER BY id_g2 ASC");
    ...
    while($row = mysqli_fetch_assoc($sql)){
        echo "<tr>
                <td class='table-head'>{$row['name_g2']}</td>
                <td class='table-head'>{$row['id_g2']}</td>
                <td class='table-head'>{$row['loc_g2']}</td>
                <td class='table-head'>{$row['course_g2']}</td>
...
if(isset($_POST['del'])){
    $sel = $_POST['se'];
    $de = mysqli_query($conn, "DELETE FROM group2 WHERE course_g2='$sel'");
...
if(isset($_POST['su'])){
    ...
    $id = "id_g2_$sel";
    $loc = "loc_g2_$sel";
    $course = "course_g2_$sel";

    $sql = mysqli_query($conn, "SELECT `$name`, `$id`, `$loc`, `$course`, tim FROM group2 WHERE (`$name`<>'' OR `$id`<>'' OR `$loc`<>'' OR `$course`<>'') AND `$course`='$co'");
...
if(isset($_POST['dell'])){
    ...
    $de = mysqli_query($conn, "DELETE FROM group2 WHERE `$course`='$co'");
