...
<div class="page-container">
    ...
    <h1 class="main-title">Table For Group 4</h1>
...
if(isset($_POST['sub'])){
    $sel = $_POST['se'];
    $sql = mysqli_query($conn, "SELECT name_g4, id_g4, loc_g4, course_g4, tim FROM group4 WHERE course_g4='$sel' ORDER BY id_g4 ASC");
    ...
    while($row = mysqli_fetch_assoc($sql)){
        echo "<tr>
                <td class='table-head'>{$row['name_g4']}</td>
                <td class='table-head'>{$row['id_g4']}</td>
                <td class='table-head'>{$row['loc_g4']}</td>
                <td class='table-head'>{$row['course_g4']}</td>
...
if(isset($_POST['del'])){
    $sel = $_POST['se'];
    $de = mysqli_query($conn, "DELETE FROM group4 WHERE course_g4='$sel'");
...
if(isset($_POST['su'])){
    ...
    $id = "id_g4_$sel";
    $loc = "loc_g4_$sel";
    $course = "course_g4_$sel";

    $sql = mysqli_query($conn, "SELECT `$name`, `$id`, `$loc`, `$course`, tim FROM group4 WHERE (`$name`<>'' OR `$id`<>'' OR `$loc`<>'' OR `$course`<>'') AND `$course`='$co'");
...
if(isset($_POST['dell'])){
    ...
    $de = mysqli_query($conn, "DELETE FROM group4 WHERE `$course`='$co'");
