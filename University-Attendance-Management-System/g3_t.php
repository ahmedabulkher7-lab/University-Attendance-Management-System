...
<div class="page-container">
    ...
    <h1 class="main-title">Table For Group 3</h1>
...
if(isset($_POST['sub'])){
    $sel = $_POST['se'];
    $sql = mysqli_query($conn, "SELECT name_g3, id_g3, loc_g3, course_g3, tim FROM group3 WHERE course_g3='$sel' ORDER BY id_g3 ASC");
    ...
    while($row = mysqli_fetch_assoc($sql)){
        echo "<tr>
                <td class='table-head'>{$row['name_g3']}</td>
                <td class='table-head'>{$row['id_g3']}</td>
                <td class='table-head'>{$row['loc_g3']}</td>
                <td class='table-head'>{$row['course_g3']}</td>
...
if(isset($_POST['del'])){
    $sel = $_POST['se'];
    $de = mysqli_query($conn, "DELETE FROM group3 WHERE course_g3='$sel'");
...
if(isset($_POST['su'])){
    ...
    $id = "id_g3_$sel";
    $loc = "loc_g3_$sel";
    $course = "course_g3_$sel";

    $sql = mysqli_query($conn, "SELECT `$name`, `$id`, `$loc`, `$course`, tim FROM group3 WHERE (`$name`<>'' OR `$id`<>'' OR `$loc`<>'' OR `$course`<>'') AND `$course`='$co'");
...
if(isset($_POST['dell'])){
    ...
    $de = mysqli_query($conn, "DELETE FROM group3 WHERE `$course`='$co'");
