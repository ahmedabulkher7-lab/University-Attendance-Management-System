...
$row = mysqli_fetch_assoc(mysqli_query($conn, "SELECT state_g2 FROM state_g2 LIMIT 1"));
// خزن القيمه بتاعت الموقع يفتح ولا لا وحط ان الافتراضى يكون صفر
$state_g2 = $row ? $row['state_g2'] : 0;
...
if ($state_g2 == 1) { // هنا لو الدكتور لالفعل حط واحد يعنى فتح الموقع
...
// لو ضغط ارسال هناخد بياناته علشان اخزنها فى الداتا بز
    if(isset($_POST['sub'])){
       ...
       // تنظيف المدخلات
        ...
        $sqll =mysqli_query($conn, "SELECT course_g2 FROM state_g2 LIMIT 1 ");
        while($row  = mysqli_fetch_assoc($sqll)){
        $course = $row ['course_g2'];
        
    }
       $sl =mysqli_query($conn, "SELECT loc_g2 FROM state_g2 LIMIT 1 ");
      ...
      // اضافة البيانات فى الداتا بيز
        ...
        $stmt = $conn->prepare("INSERT INTO group2 (name_g2, id_g2, course_g2, loc_g2) VALUES (?, ?, ?, ?)");
 ...
 // تعريف البعد المتاح  قبل استخدامه
    ...
    $lo = mysqli_query($conn, "SELECT loc_g2 FROM state_g2 LIMIT 1");
...
<head>
    ...
    <link rel='shortcut icon' href='https://i.postimg.cc/0y44MsH1/Picsart-25-11-27-23-47-53-304.png' type='image/x-icon' />
            ...
            <body>
            <div class='container'>
        <h1>Sections Group 2</h1>
        <div class='sections-container'>
      <img src='section.jpg' alt='sections' class='sections-img'>
      <div class='buttons-box'>
        <h1 class='page-title'>Sections — Group 2</h1>
        <a href='g2_c1.php'><button class='section-btn'>Section 1</button></a>
        <a href='g2_c2.php'><button class='section-btn'>Section 2</button></a>
        <a href='g2_c3.php'><button class='section-btn'>Section 3</button></a>
        <a href='g2_c4.php'><button class='section-btn'>Section 4</button></a>
      </div>
    </div>
      </body>
</html>
